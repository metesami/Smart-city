#pip install rdflib # maybe needed
import pandas as pd
...
for _, row in metadata_df.iterrows():
    ...
    way_id = str(way_id).strip()
    if way_id not in street_uri_map:
        street_uri = EX[f"way_{way_id}"]
    ...
    #Boolean parsing for bicycle_dedicated_lane
    def parse_bool(v):
    ...
    # Connected roads as object URIs
    '''if connected_roads:
        for cr in str(connected_roads).split(","):
            cr = cr.strip()
            if cr:
                connected_uri = EX[f"way_{cr}"]
                g.add((sensor_uri, CTDO.connectedRoads, connected_uri))
'''
    # Link Sensor to Lane
